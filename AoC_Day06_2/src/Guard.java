...
public class Guard {
	...
	public Guard(Map map, GridVector gridVector, int stuckThreshhold) {
		this.map = map;
		...
		this.steps = 0;
		this.stuckThreshhold = stuckThreshhold;
	}
	...
	public boolean walk() {
		...
		if (newX < 0 || newX >= this.map.getWidth() || newY < 0 || newY >= this.map.getHeight()) {
			this.x = newX;
			this.y = newY;
	...
	public boolean isStuck() {
		return this.steps > this.stuckThreshhold;
	}
