...
public class Grid {
	...
	public List<Grid> getQuadrants(){
		...
		List<Tile> tileList = this.tileList.stream().filter(tile -> tile.getX() < firstWidth && tile.getY() < firstHeight).toList();
		Grid grid = Grid.fromList(tileList);
		gridList.add(grid);
		
		tileList = this.tileList.stream().filter(tile -> tile.getX() > firstWidth && tile.getY() < firstHeight).toList();
