...
public class Grid {
	...
	public void addTile(Tile tile) {
		...
		if (x < 0 || y < 0 || x >= this.width || y >= this.height)
			throw new IndexOutOfBoundsException(
					"Index is out of bounds for x = %d and y = %d with grid of width %d and height %d"
					.formatted(x, y, this.width, this.height));
	...
	public Tile getTile(int index) {
		if (index < 0 || index >= this.tileList.size())
			throw new IndexOutOfBoundsException(
					"Index is out of bounds for index = %d with list of size %d"
